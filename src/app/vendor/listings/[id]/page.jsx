...
const GRAD = ["linear-gradient(135deg,#0e7490,#4f46e5)", "linear-gradient(135deg,#0ea5a4,#2f6fed)", "linear-gradient(135deg,#f59e0b,#ec4899)"];
const TYPE_COLORS = {
...
export default function VendorListingDetailPage() {
  ...
  // Guard: only the owning vendor may view their listing here.
  if (!listing || listing.vendorId !== vendorId) {
    return (
      <Box>
        <Button startIcon={<ArrowBackRoundedIcon />} color="inherit" onClick={() => router.push("/vendor/listings")}>Back to my listings</Button>
        <Card sx={{ p: 6, mt: 2, textAlign: "center" }}>
          <Typography color="text.secondary">Listing not found in your storefront.</Typography>
        </Card>
      </Box>
    );
  }
  ...
  return (
    <Box>
      <Stack direction="row" sx={{ justifyContent: "space-between", alignItems: "center", mb: 2 }}>
        <Button startIcon={<ArrowBackRoundedIcon />} color="inherit" onClick={() => router.push("/vendor/listings")}>My Listings</Button>
        <Stack direction="row" spacing={1}>
          ...
          <Button variant="outlined" color="error" startIcon={<DeleteOutlineRoundedIcon />} onClick={() => { dispatch(listingsActions.remove(listing.id)); dispatch(notify({ message: "Listing deleted", severity: "info" })); router.push("/vendor/listings"); }}>Delete</Button>
        </Stack>
      </Stack>

      {/* Hero */}
      <Card sx={{ overflow: "hidden", mb: 3 }}>
        <Box sx={{ height: 100, background: GRAD[listing.title.length % GRAD.length] }} />
        <Box sx={{ px: { xs: 2, md: 3 }, pb: 3 }}>
          <Stack direction={{ xs: "column", sm: "row" }} spacing={2} sx={{ alignItems: { sm: "flex-end" }, mt: -4 }}>
            <Avatar variant="rounded" sx={{ width: 78, height: 78, borderRadius: 3, fontSize: 26, fontWeight: 700, bgcolor: "secondary.main", border: "4px solid", borderColor: "background.paper" }}>{listing.title[0]}</Avatar>
            <Box sx={{ flex: 1, pb: 0.5 }}>
              ...
              <Stack direction="row" spacing={0.75} sx={{ alignItems: "center", mt: 0.5, color: "text.secondary", flexWrap: "wrap" }}>
                ...
                <Typography variant="body2" color="text.secondary">{cat?.name}{sub ? ` · ${sub.name}` : ""} · {listing.city}</Typography>
              </Stack>
        ...
        <Stack spacing={3}>
          <Card sx={{ p: { xs: 2, md: 3 } }}>
            ...
            <Table>
              <TableBody>
                {typeChips(listing).map((c) => (
                  <TableRow key={c.type}>
                    <TableCell sx={{ width: 120 }}><Chip label={c.label} size="small" sx={{ fontWeight: 700, bgcolor: TYPE_COLORS[c.type]?.bg, color: TYPE_COLORS[c.type]?.fg }} /></TableCell>
          ...
          <Card sx={{ p: { xs: 2, md: 3 } }}>
            <Stack direction="row" sx={{ justifyContent: "space-between", alignItems: "center", mb: 1.5 }}>
              ...
              <Button size="small" color="inherit" onClick={() => router.push("/vendor/inquiries")}>My inquiries</Button>
            </Stack>
            <Table>
              <TableHead>
                <TableRow><TableCell>Customer</TableCell><TableCell>Type</TableCell><TableCell>Event date</TableCell><TableCell align="right">Status</TableCell></TableRow>
              </TableHead>
              <TableBody>
                ...
                {listingInquiries.length === 0 && <TableRow><TableCell colSpan={4} align="center" sx={{ py: 3, color: "text.secondary" }}>No inquiries yet.</TableCell></TableRow>}
              </TableBody>
            </Table>
          </Card>

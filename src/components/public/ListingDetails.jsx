...
export default function ListingDetails({ publicView = false }) {
  const { id } = useParams();
  ...
  const browsePath = publicView ? "/listings" : "/user/browse";
  const loginToEnquire = () => router.push(`/user/login?next=${encodeURIComponent(`/listings/${id}`)}`);
  const requiresLogin = publicView && !myEmail;

  if (!listing || listing.status !== "Published") {
    return (
      <Box>
        <Button startIcon={<ArrowBackRoundedIcon />} color="inherit" onClick={() => router.push(browsePath)}>Browse</Button>
  ...
  return (
    <Box sx={publicView ? { maxWidth: 1440, mx: "auto", px: { xs: 2, md: 3 }, py: { xs: 4, md: 6 } } : undefined}>
      <Button startIcon={<ArrowBackRoundedIcon />} color="inherit" sx={{ mb: 2 }} onClick={() => router.push(browsePath)}>Browse</Button>
      ...
      <Box sx={{ display: "grid", gap: 3, gridTemplateColumns: { xs: "1fr", lg: "2fr 1fr" } }}>
            ...
            <Card sx={{ p: { xs: 2, md: 3 } }}>
              ...
              <Box sx={{ mt: 2 }}>
                <HallAvailabilityCalendar
                  ...
                  onPickSlot={(date, slot) => requiresLogin ? loginToEnquire() : setAsk({ eventDate: date, slot })}
                />
              </Box>
            </Card>
          ) : (
        ...
        <Stack spacing={3}>
          ...
          <Card sx={{ p: { xs: 2, md: 3 }, position: { lg: "sticky" }, top: 24 }}>
            ...
            <Button fullWidth size="large" variant="contained" startIcon={<SendRoundedIcon />} sx={{ mt: 2 }} onClick={requiresLogin ? loginToEnquire : () => setAsk({})}>
              {requiresLogin ? "Sign in to enquire" : "Send inquiry"}
            </Button>
